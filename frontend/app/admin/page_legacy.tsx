        ...
        <div className="hero-panel" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 'var(--spacing-base)' }}>
          <div className="stat-card">
            ...
            <strong style={{
              ...
              {formConfig?.dynamicStatus === 'OPEN' ? '≡ƒƒó Open' :
               formConfig?.dynamicStatus === 'PAUSED' ? 'ΓÅ╕ Paused' :
               formConfig?.dynamicStatus === 'CLOSED' ? '≡ƒö┤ Closed' :
               formConfig?.dynamicStatus === 'LIMIT_REACHED' ? '≡ƒö┤ Limit Reached' :
               formConfig?.dynamicStatus === 'SCHEDULED' ? 'ΓÅ│ Scheduled' :
               formConfig?.dynamicStatus === 'ARCHIVED' ? '≡ƒôª Archived' : '≡ƒƒí Draft'}
            </strong>
          </div>
          <div className="stat-card">
            <span>Responses Capacity</span>
            <strong style={{ fontSize: '1.3rem' }}>
              {formConfig?.submissionCount ?? 0} / {formConfig?.config?.maxResponses > 0 ? formConfig.config.maxResponses : 'Γê₧'}
            </strong>
          </div>
          ...
          <div className="hero-note" style={{ gridColumn: '1 / -1' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', fontSize: '0.8rem', color: 'var(--muted)' }}>
              <span>≡ƒôà <strong>Scheduled Open:</strong> {formConfig?.config?.openAt ? new Date(formConfig.config.openAt).toLocaleString() : 'Manual'}</span>
              <span>≡ƒôà <strong>Scheduled Close:</strong> {formConfig?.config?.closeAt ? new Date(formConfig.config.closeAt).toLocaleString() : 'Manual'}</span>
              <span>≡ƒ¢í∩╕Å <strong>Access mode:</strong> {formConfig?.config?.accessMode || 'PUBLIC'}</span>
            </div>
          </div>
        </div>
      </section>
